...
function NewInvoice() {
  ...
  const filteredLibrary = useMemo(() => {
    ...
    return list
      .filter((i: any) =>
        `${i.sku ?? ""} ${i.name ?? ""} ${i.brand ?? ""} ${i.category ?? ""}`
          .toLowerCase()
          .includes(q),
      )
      .slice(0, 50);
  ...
  async function createBike() {
    ...
    if (!nbMake.trim() || !nbModel.trim()) {
      toast.error("Make and model required");
      return;
    }
  ...
  async function save(postAction: "view" | "print" | "email" = "view") {
    ...
    const subInc = cleanLines.reduce(
      (s, l) => s + l.unit * l.quantity * (1 - l.discount_pct / 100),
      0,
    );
  ...
  async function persistDraftIfNeeded() {
    ...
    const hasContent = !!s.customerId || cleanLines.length > 0 || s.notes.trim().length > 0;
    ...
    <div className="space-y-5 max-w-5xl mx-auto">
      <header className="flex items-start gap-3">
        <button
          ...
          className="grid h-9 w-9 place-items-center rounded-lg border border-border mt-1"
        >
        ...
        {selectedCustomer ? (
          ...
          <div>
            <div className="relative">
              ...
              <Input
                className="pl-9"
                placeholder="Search customers (leave blank for walk-in)"
                value={search}
      ...
      <section className="card-surface p-4 space-y-3">
        ...
        {!customerId ? (
          <div className="text-xs text-muted-foreground">
            Select a customer above to assign a bike.
          </div>
        ) : (
          <>
            {showNewBike && (
      ...
      <section className="card-surface p-4">
        ...
        <div className="mt-4 pt-3 border-t border-border space-y-1 text-sm max-w-xs ml-auto">
          ...
          <div className="flex justify-between pt-1.5 border-t border-border font-display text-lg font-black">
            <span>TOTAL</span>
            <span className="red-gradient-text tabular-nums">${total.toFixed(2)}</span>
          </div>
        </div>
      </section>
