...
function ClockPage() {
  ...
  const openJobs = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("jobs")
        .select(
          "id, job_number, complaint, status, motorcycles(make, model, rego), customers(first_name, last_name)",
        )
    ...
    if (type === "clock_out") {
      ...
      for (const activeEntry of activeEntries ?? []) {
        const minutes = Math.max(
          1,
          Math.round((+ended - +new Date(activeEntry.started_at)) / 60000),
        );
        const { error: timerError } = await supabase
          .from("time_entries")
          .update({ ended_at: ended.toISOString(), minutes })
  ...
  async function clockInFromSearch() {
    ...
    let pick =
      (openJobs.data ?? []).find((j: any) => String(j.job_number) === q) ??
  ...
  const filteredJobs = (openJobs.data ?? []).filter((j: any) => {
    ...
    const cust = j.customers
      ? `${j.customers.first_name ?? ""} ${j.customers.last_name ?? ""}`.toLowerCase()
      : "";
    return (
      String(j.job_number).includes(q) ||
      bike.includes(q) ||
      rego.includes(q.replace(/\s+/g, "")) ||
      cust.includes(q) ||
    ...
    <div className="space-y-5 max-w-2xl mx-auto">
      <header>
        ...
        {user && (
          <div className="text-sm text-muted-foreground mt-1">
            Logged in as{" "}
            <span className="font-semibold text-foreground">{fullName || user?.email}</span>
          </div>
        )}
      </header>
          ...
          <div className="text-sm flex-1">
            ...
            {((activeJob.data as any).motorcycles ?? (activeJob.data as any).bikes) && (
              <span className="text-muted-foreground">
                {" "}
                · {((activeJob.data as any).motorcycles ?? (activeJob.data as any).bikes).make}{" "}
                {((activeJob.data as any).motorcycles ?? (activeJob.data as any).bikes).model}
              </span>
            )}
          </div>
        </Link>
      ...
      <div className="grid grid-cols-2 gap-3">
        {state === "off" && (
          <Button
            ...
            className="col-span-2 h-20 text-lg gold-surface font-bold gap-2"
          >
        ...
        {state === "on" && (
          <>
            <Button
              onClick={() => add("break_start")}
              className="h-20 text-base font-bold gap-2 bg-secondary text-foreground hover:bg-secondary/80"
            >
              <Coffee className="h-5 w-5" /> Start Break
            </Button>
            <Button
              onClick={() => add("clock_out")}
              className="h-20 text-base font-bold gap-2 bg-status-parts hover:bg-status-parts/90 text-white"
            >
        ...
        {state === "break" && (
          <>
            <Button
              onClick={() => add("break_end")}
              className="h-20 text-base font-bold gap-2 gold-surface"
            >
      ...
      <section className="card-surface p-4">
        ...
        <div className="space-y-1.5">
          ...
          {(!events.data || events.data.length === 0) && (
            <p className="text-sm text-muted-foreground">No clock activity yet.</p>
          )}
        </div>
      </section>
      ...
      <Dialog open={pickingJob} onOpenChange={setPickingJob}>
        <DialogContent className="max-w-lg">
              ...
              >
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {j.customers
                    ...
                    : ""}
                  {(j.motorcycles ?? j.bikes)
                    ? ` · ${(j.motorcycles ?? j.bikes).make ?? ""} ${(j.motorcycles ?? j.bikes).model ?? ""}`
                    : ""}
                </div>
                {j.complaint && (
                  <div className="text-xs text-foreground/80 truncate mt-0.5">{j.complaint}</div>
                )}
              </button>
            ))}
          </div>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setPickingJob(false)}>
              Cancel
            </Button>
            <Button onClick={clockInFromSearch} disabled={!jobQuery.trim()}>
              Clock In
            </Button>
          </DialogFooter>
        </DialogContent>
