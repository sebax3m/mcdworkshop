  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select(
          "id, service_type, service_type_other, scheduled_date, drop_off_time, estimated_hours, status, confirmed, bike_arrived, bike_arrived_at, pickup_required, delivery_required, transport_address, loan_bike, job_id, notes, complaints, rego, customer_id, motorcycle_id, assigned_tech_id, customers(first_name,last_name,phone), motorcycles(year,make,model,rego,photos), jobs(id,status)",
        )
