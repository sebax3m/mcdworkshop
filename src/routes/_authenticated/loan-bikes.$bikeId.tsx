...
function LoanBikeDetail() {
  ...
  const assignmentsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select(
          "id, scheduled_date, loan_bike_expected_return, loan_bike_returned_at, loan_bike_start_km, loan_bike_end_km, job_id, status, customers(first_name,last_name,phone), motorcycles(make,model,year)",
        )
  ...
  async function addNote() {
    ...
    try {
      const { error } = await supabase
        .from("loan_bike_notes")
        .insert({ loan_bike_id: bikeId, note: note.trim() });
      if (error) throw error;
      setNote("");
      notesQ.refetch();
    } catch (e: any) {
  ...
  if (!bike)
    return <div className="card-surface p-8 text-center text-muted-foreground">Not found.</div>;
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      <header className="flex items-center gap-3">
        ...
        <button
          onClick={openEdit}
          className="text-xs font-semibold rounded-lg border border-border px-3 h-9"
        >
          Edit
        </button>
        <button
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {active.loan_bike_expected_return && (
                <span className="inline-flex items-center gap-1 text-xs text-amber-400">
                  <CalendarClock className="h-3 w-3" /> Return by{" "}
                  {format(new Date(active.loan_bike_expected_return + "T00:00:00"), "d MMM")}
                </span>
              )}
              {active.job_id && (
                <Link
                  to="/jobs/$jobId"
                  params={{ jobId: active.job_id }}
      ...
      <section className="card-surface p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          />
          <Input
            placeholder="Description (oil change…)"
            value={logDesc}
        ...
        <Button onClick={addLog} className="w-full sm:w-auto">
      ...
      <section className="card-surface p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <Textarea
            ...
            placeholder="Anything to remember about this bike…"
            rows={2}
          />
                ...
                <span className="text-xs text-muted-foreground">
                  ...
                  {a.loan_bike_returned_at
                    ...
                    : a.loan_bike_expected_return
                      ? ` · exp. return ${format(new Date(a.loan_bike_expected_return + "T00:00:00"), "d MMM")}`
                      : ""}
                </span>
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                {a.loan_bike_start_km != null && (
                  <>Start: {Number(a.loan_bike_start_km).toLocaleString()} km </>
                )}
                {a.loan_bike_end_km != null && (
                  <>· End: {Number(a.loan_bike_end_km).toLocaleString()} km</>
                )}
                {a.job_id && (
                  <Link
                    to="/jobs/$jobId"
      ...
      <AlertDialog open={editing} onOpenChange={setEditing}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Update this bike's details, current kilometres and service schedule.
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the bike and all of its service logs and notes. Booking history stays
              intact.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
