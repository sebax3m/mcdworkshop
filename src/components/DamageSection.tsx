...
export function DamageSection({
  ...
  const [severity, setSeverity] = useState<Mark["severity"]>("moderate");
  ...
  const photos = useQuery({
    queryKey: ["job-damage-photos", jobId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("job_photos")
        .select("id, storage_path, caption, created_at")
        .eq("job_id", jobId)
        .ilike("caption", `${PRINT_PREFIX}%`)
  ...
  async function saveMarks() {
    ...
    try {
      const { data: cur } = await supabase
        ...
        .eq("id", jobId)
        .maybeSingle();
      const sd = ((cur as any)?.service_data ?? {}) as any;
      sd.damage_marks = marks;
      const { error } = await supabase.from("jobs").update({ service_data: sd }).eq("id", jobId);
      if (error) throw error;
      setDirty(false);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const f of files) {
        const path = await uploadPhoto(f, `damage/${jobId}`);
        const { error } = await supabase.from("job_photos").insert({
          job_id: jobId,
          uploaded_by: uid,
          storage_path: path,
          caption: `${PRINT_PREFIX}${f.name}`,
        } as any);
        if (error) throw error;
      }
  ...
  return (
    <section className="card-surface p-4 sm:p-5 border-l-4 border-orange-500/60 print:break-inside-avoid">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        ...
        {canEdit && dirty && (
          <Button onClick={saveMarks} disabled={saving} size="sm" className="gold-surface gap-2">
            <Save className="h-3.5 w-3.5" /> {saving ? "Saving…" : "Save diagram"}
          </Button>
        )}
      </div>

      {/* View + severity controls */}
      <div className="flex items-center gap-2 flex-wrap mb-3 print:hidden">
        ...
        {canEdit && (
          <div className="inline-flex rounded-lg border border-border p-0.5">
            {(Object.keys(SEVERITY) as Mark["severity"][]).map((s) => (
              <button
                key={s}
          ...
          {viewMarks.map((m, i) => (
            <g key={m.id} className="pointer-events-auto">
              <circle
                ...
                cy={m.y * 320}
                r={14}
                fill={SEVERITY[m.severity].color}
                fillOpacity={0.85}
                stroke="white"
                strokeWidth={2}
              />
              ...
              >
                ...
                {canEdit && (
                  <button
                    ...
                    className="absolute top-1 right-1 grid h-7 w-7 place-items-center rounded-full bg-background/80 backdrop-blur text-destructive opacity-0 group-hover:opacity-100 transition print:hidden"
                  >
